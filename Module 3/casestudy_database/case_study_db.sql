...
left join loai_dich_vu ldv  on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu
where (dv.ma_dich_vu not in ( select hop_dong.ma_dich_vu from hop_dong 
where year(hop_dong.ngay_lam_hop_dong) = 2021)) and (dv.ma_dich_vu in (select hop_dong.ma_dich_vu from hop_dong 
...
-- 9. Thực hiện thống kê doanh thu theo tháng, nghĩa là tương ứng với mỗi tháng trong năm 2021 thì sẽ có bao nhiêu khách hàng thực
--    hiện đặt phòng.
...
select hd.ma_hop_dong , hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,hd. tien_dat_coc,sum(hop_dong_chi_tiet.so_luong) as so_luong_dich_vu_di_kem from hop_dong hd
...
select hop_dong.ma_hop_dong,nhan_vien.ho_ten,khach_hang.ho_ten,khach_hang.so_dien_thoai,hop_dong.ma_dich_vu,dich_vu.ten_dich_vu,sum(hop_dong_chi_tiet.so_luong)
...
and hop_dong.ma_hop_dong in (select hop_dong.ma_hop_dong from hop_dong 
